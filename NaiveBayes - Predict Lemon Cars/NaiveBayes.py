...
def predict(X,attributes, values, length, class_prob, p_conditionals):
  ...
  for i in range(length):
      if p_conditionals.has_key(attributes[i]) and p_conditionals[attributes[i]].has_key(float(values[i])):
...
def call_predict(X, attributes):
    ...
    for x in X:
        ...
        if probability > 0.5 and y[0] == '1':
            true_positive += 1
            decision[count] = 1
        elif probability <= 0.5 and y[0] == '0':
            true_negative += 1
            decision[count] = 0
        elif probability > 0.5 and y[0] == '0':
            false_positive += 1
...
X = open("training.dat").read()
A = open("test.dat").read()
